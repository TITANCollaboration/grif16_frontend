...
int frontend_init()
{
   ...
   if(      strcmp(tmp_str, "grifip09") == 0 ||
            ...
            strcmp(tmp_str, "grifadc75b") == 0 ||
            strcmp(tmp_str, "grifadc17") == 0 ||
            strcmp(tmp_str, "grifadc60") == 0 ||
            strcmp(tmp_str, "grifadc19") == 0 ||
            strcmp(tmp_str, "grifadc21") == 0 ||
            strcmp(tmp_str, "grifadc22") == 0 ){ /*grif16*/; }
   else {
...
int read_grifc_event(char *pevent, int grifc_id)
{
   ...
   if( multi_evt_pos<TARGET_EVENT_SIZE && curr_time==last_event_update &&
       multi_evt_pos+8*EVTBUFSIZ < MAX_EVT_SIZE ){
   ...
   bk_create(pevent, "GRF4", TID_DWORD, (void**)&pdata);
...
int open_udp_socket(char *host, int server_port, struct sockaddr_in *addr)
{
   ...
   local_addr.sin_port = htons(server_port);          // 0 => use any available port   local_addr.sin_addr.s_addr = INADDR_ANY; // listen to all local addresses
